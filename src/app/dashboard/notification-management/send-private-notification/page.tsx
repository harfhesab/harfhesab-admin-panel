...
const Page = () => {
  ...
  const registerAndConfirm = () => {
    if (!userSelected?._id) {
      toast.error("کاربری برای ارسال اعلان خصوصی انتخاب نکرده‌اید.", toastOptions());
    ...
    await axios({
      ...
      .then(async (response) => {
        ...
        if (response.data?.data?.registerNewPrivateNotificationByAdmin?.status == 200) {
          toast.success(response.data?.data?.registerNewPrivateNotificationByAdmin?.message, { ...toastOptions(), autoClose: 3000 });
  ...
  const selectUser = () => {
    ...
    UserListHelper.openModal({
      ...
      numberSelected: 1,
      buttons: [
        { buttonText: "لغو", type: "border", onClickFn: () => { UserListHelper.closeModal(); } },
        {
          buttonText: "انتخاب کاربر",
          type: "bold",
          onClickFn: ({ data }: { data: any }) => {
            setUserSelected({ _id: data._id[0], user_name: data.user_name[0], phone: data?.phone[0], name: data?.name[0] })
          ...
          <div className="flex flex-col items-center gap-2 bg-background3 dark:bg-background3_dark border border-dashed border-info dark:border-info rounded-md py-4">
            <div className="relative w-[90%] h-22 3xs:h-24 sm:h-32 cursor-pointer">
              ...
              <div className="absolute top-0 w-full flex justify-between px-1 pt-1">
                <div
                  onClick={(e: any) => { e.stopPropagation(); deleteUserItem(); }}
...
pointer-events-none inline-block h-[22px] w-[22px] transform rounded-full shadow-lg ring-0 transition duration-200 ease-in-out`}
      ...
      <UserList ref={(Ref) => { UserListHelper.setRef(Ref); }} />
